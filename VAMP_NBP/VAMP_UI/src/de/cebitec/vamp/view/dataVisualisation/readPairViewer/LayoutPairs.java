package de.cebitec.vamp.view.dataVisualisation.readPairViewer;

import de.cebitec.vamp.databackend.dataObjects.PersistantMapping;
import de.cebitec.vamp.databackend.dataObjects.PersistantReadPairGroup;
import de.cebitec.vamp.databackend.dataObjects.PersistantReadPair;
...
public class LayoutPairs implements LayoutI {
    ...
    /**
     * Creates a new layout for sequence pairs.
     * @param absStart start of the interval
     * @param absStop end of the interval
     * @param seqPairs all sequence pairs to add to the layout
     */
    public LayoutPairs(int absStart, int absStop, Collection<PersistantReadPairGroup> seqPairs) {
     ...
     */
    private void createBlocks(Collection<PersistantReadPairGroup> seqPairList) {
        Iterator<PersistantReadPairGroup> groupIt = seqPairList.iterator();
        while (groupIt.hasNext()) {
            PersistantReadPairGroup group = groupIt.next();
            List<PersistantReadPair> seqPairs = group.getReadPairs();
            List<PersistantMapping> singleMappings = group.getSingleMappings();
            Iterator<PersistantReadPair> pairIt = seqPairs.iterator();
