...
// the main() function
    public static void main(String[] args) {
        String relation = "/vol/vamp/DevelopmentStuff/TestingVamp/BAM/testGlut.sam"; 
...
//    }
    ...
    private static void externalSort(String relation) {
        try {
            //file input
            FileReader intialRelationInput = new FileReader(relation);
...
// get 10k rows
                for (int i = 0; i < 10000; ) {
                    ...
                    tenKRows.add(row);
                        ...
                        header = header.concat(initRelationReader.readLine()+"\n");
                    }
                }
// sort the rows
...
// merge the the results for mergeSort back together
    private static ArrayList<String[]> merge(ArrayList<String[]> left, ArrayList<String[]> right, int index) {
        ...
        if (right.size() > 0) {
         ...
         java.util.Arrays.sort(result.toArray());
        return result;
    }
