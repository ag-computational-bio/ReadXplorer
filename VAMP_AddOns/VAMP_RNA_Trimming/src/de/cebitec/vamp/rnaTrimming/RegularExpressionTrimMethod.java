...
public class RegularExpressionTrimMethod extends TrimMethod {
    ...
    public RegularExpressionTrimMethod(String regularexpression, int groupnumber_main, int groupnumber_trimLeft, int groupnumber_trimRight, String name) {
    ...
    public static RegularExpressionTrimMethod createNewInstance(Type t) {
        if (t.equals(Type.VARIABLE_RIGHT)) 
            return new RegularExpressionTrimMethod("^(.*?)(A{0,%X%})$", 1, GROUPNUMBER_UNUSED, 2, "trim poly-A from 3' end (right to left) by variable length");
        else if (t.equals(Type.VARIABLE_LEFT)) 
            return new RegularExpressionTrimMethod("^(A{0,%X%})(.*?)$", 2, 1, GROUPNUMBER_UNUSED, "trim poly-A from 5' end (left to right) by variable length");
        else if (t.equals(Type.VARIABLE_BOTH)) 
            return new RegularExpressionTrimMethod("^(A{0,%X%})(.*?)(A{0,%X%})$", 2, 1, 3, "trim poly-A from 3' and from 5' end by variable length");
        else if (t.equals(Type.FIXED_RIGHT)) 
            return new RegularExpressionTrimMethod("^(.*?)(.{%X%})$", 1, GROUPNUMBER_UNUSED, 2, "trim all nucleotides from 3' end (right to left) by fixed length");
        else if (t.equals(Type.FIXED_LEFT)) 
            return new RegularExpressionTrimMethod("^(.{%X%})(.*?)$", 2, 1, GROUPNUMBER_UNUSED, "trim all nucleotides from 5' end (left to right) by fixed length");
        else if (t.equals(Type.FIXED_BOTH)) 
            return new RegularExpressionTrimMethod("^(.{%X%})(.*?)(.{%X%})$", 2, 1, 3, "trim all nucleotides from 3' and from 5' end by fixed length");
        
        else return new RegularExpressionTrimMethod("^(.{%X%})(.*?)(.{%X%})$", 2, 1, 3, "UNKNOWN");
    }
}
