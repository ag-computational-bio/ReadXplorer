...
final class GeneticCodePanel extends OptionsPanel {
    ...
    /**
     * Adds a custom genetic code taken from the customCodonField to both, the
     * gui list and the custom genetic codes storage.
     * <br>Input format: "(startcodon1, startcodon2, startcodon3, ...;
     * stopcodon1, stopcodon2, ...) - codeName"
     */
    ...
    /**
     * Adds a custom genetic code string to the gui table model. Input format:
     * "(codon1, codon2, codon3, ...) - codeName"
     * <p>
    ...
    /**
     * Removes the selected custom genetic code from both, the gui list and the
     * custom genetic codes storage.
     */
    private void removeCodonsFromList() {
        ...
        int codeIndex = this.geneticCodeList.getSelectedIndex();
        int officialCodeIdx = genCodes.size();
        int endIndex = 0;
        int lineBreakIndex;

        //remove from storage
        if( codeIndex >= officialCodeIdx ) {

            if( codeIndex == Integer.valueOf( pref.get( Properties.GENETIC_CODE_INDEX, "0" ) ) ) {
                //reset genetic code to standard
                pref.put( Properties.SEL_GENETIC_CODE, String.valueOf( genCodes.get( 0 ).getId() ) );
                pref.put( Properties.GENETIC_CODE_INDEX, "0" );
            }

            int ownCodeIdx = officialCodeIdx;
            while( ownCodeIdx++ < codeIndex ) {
            ...
            pref.put( Properties.CUSTOM_GENETIC_CODES, origCustomCodes.substring( 0, endIndex - 1 ) + customCodes );

            //remove from table model
            ((GeneticCodeListModel) this.geneticCodeList.getModel()).removeElement( codeIndex );
            geneticCodeList.setSelectedIndex( 0 );
        }
    }


    /**
     * Internal class representing the table model for start codons. Implements
     * all methods needed for handling some geneticCodesData.
     */
