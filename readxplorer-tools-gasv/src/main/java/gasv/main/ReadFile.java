...
public abstract class ReadFile {
    ...
    /* ReadFile is a generic class that contains all basic functions of reading
     * a data file and creating geometric structures. ReadESP, ReadCGH, and
     * ReadPred extend from this class.
     */
    ...
    protected int chrFormatToNumber( String ch ) throws NumberFormatException {
        ...
        if( ch.length() >= 3 && ch.substring( 0, 3 ).equals( "chr" ) ) {
            if( ch.substring( 3, 4 ).equals( "X" ) ) {
                ...
                toReturn = Integer.parseInt( ch.substring( 3, ch.length() ) );
            }
    ...
    /**
     * Subclasses must implement this abstract method. Does the work of reading
     * in the data from the file and converting each line of data into a type of
     * BreakRegion. The BreakRegion is then placed in the ArrayList<BreakRegion>
     * data structure that was passed into the constructor as long as it matches
     * the specified leftChr and rightChr.
     */
     ...
     * in the data from the file and converting each line of data into a type of
     * BreakRegion. The BreakRegion is then placed in the ArrayList<BreakRegion>
     * data structure that was passed into this method call as long as it
     * matches the specified leftChr and rightChr.
     * <p>
     * Only read in those clones that match the specified targetLeftChr and
     * targetRightChr and are within the current window as specified by
