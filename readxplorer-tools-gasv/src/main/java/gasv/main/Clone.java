...
public class Clone extends BreakRegion {
    ...
    public Poly getPoly() {
        if( p == null ) {
            ...
            if( isTriangle() ) {
				...
				/* if (xLen == yLen) { Lmin = (2 * xLen) + 1; }
                 * else { Lmin = (int)(Math.sqrt((2 * xLen * xLen) + (2 * yLen *
                 * yLen)) + 1.0d);
                 *
                 * }
                 * Out.print2("WARNING!!! Encountered triangle condition for
                 * ESP: " + name + ", so increasing Lmin for this ESP to " +
                 * Lmin);
                 */
                Lmin = xLen + yLen;
			...
			/* int xLenTrimmed = xLen - 5; int yLenTrimmed = yLen - 5; //assume
             * any read starting at coordinate 0 must be positive! int signumX =
             * (int) Math.signum(x); if (signumX == 0) { signumX = 1; } int
             * signumY = (int) Math.signum(y); if (signumY == 0) { signumY = 1;
             * }
             *
             * int signedXLen = signumX * xLenTrimmed; int signedYLen = signumY
             * * yLenTrimmed; double newX = Math.abs(x) + signedXLen; double
             * newY = Math.abs(y) + signedYLen;
             *
             * p.add(Math.abs(x) + signumX * Lmin - signumX * yLenTrimmed,
             * newY); p.add(Math.abs(x) + signumX * Lmax - signumX *
             * yLenTrimmed, newY); p.add(newX, Math.abs(y) + signumY * Lmax -
             * signumY * xLenTrimmed); p.add(newX, Math.abs(y) + signumY * Lmin
             * - signumY * xLenTrimmed);
             */
        }
        return p;
    ...
    public void setEdges() {
        ...
        /*
         * double ycoord = Math.abs(y); double ycoordMin = Math.abs(y) +
         * Math.signum(y)*Lmin; double ycoordMax = Math.abs(y) +
         * Math.signum(y)*Lmax;
         *
         * double xcoord = Math.abs(x); double xcoordMin = Math.abs(x) +
         * Math.signum(x)*Lmin; double xcoordMax = Math.abs(x) +
         * Math.signum(x)*Lmax;
         */
    ...
    public void setBmin( int min ) {
        bmin = min;
    }


    public void setBmax( int max ) {
        bmax = max;
    }
     ...
     */
    /* public void shift() { //nextDouble() returns rand double between 0.0 and
     * 1.0 // multiply by MAX_SHIFT to get random value between 0.0 and
     * MAX_SHIFT double xShift = rand_.nextDouble() * Constants.MAX_SHIFT;
     * double yShift = rand_.nextDouble() * Constants.MAX_SHIFT;
     *
     * //ensure shifts are at least 0.1... This will skew the //distribution
     * somewhat, but shouldn't matter double threshold =
     * Constants.ZERO_THRESHOLD; if (xShift < threshold) { xShift += threshold;
     * } if (yShift < threshold) { yShift += threshold; }
     *
     * x += xShift; y += yShift; }
     */
